...
function ServiceCard({ icon, title, className }: ServiceCardProps) {
    return (
        <Card className={cn('relative group rounded-3xl overflow-hidden border-none shadow-none cursor-pointer h-full', className)}>
            <motion.div className="absolute inset-0 m-auto w-20 h-20 bg-primary scale-1 group-hover:scale-[8] opacity-0 group-hover:opacity-100 rounded-full transition-transform duration-700"/>
            <CardContent className='p-6 md:p-10 xl:p-12 relative h-full grid content-between'>
