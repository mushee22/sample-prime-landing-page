...
function ServiceCard({ icon, title, className, id }: ServiceCardProps) {
    return (
        <Link href={`/services#${id}`} key={id} >
            <Card className={cn('relative group rounded-3xl overflow-hidden border border-[#0C0A011F] shadow-none cursor-pointer h-full', className)}>
                ...
                <CardContent className='p-6 md:py-12 relative h-full grid content-between'>
                    <Image src={icon} alt={title} className='mx-auto size-16 group-hover:rotate-[120deg] ease-linear duration-300' />
